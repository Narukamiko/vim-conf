...
hi def link cAnsiFunction customAnsiFunction
"hi def link cAnsiName cIdentifier

syn match customArithOperator "\s[-+*/%]\s"
syn match customArithOperator "\s++\|\s--"
syn match customArithOperator "++\s\|--\s"
syn match customArithOperator "\s[><]\s"
syn match customArithOperator "\s[><=!]=\s"

syn match customBitOperator "\s[&^|~]\s"
syn match customBitOperator "\s<<\s\|\s>>\s"

syn match customComOperator "\s&&\s\|\s||\s"
syn match customComOperator "\s?\s\|\s\:\s"
syn match customComOperator "!"

syn match customAssignOperator "\s=\s"
syn match customAssignOperator "\s<<=\s\|\s>>=\s"
syn match customAssignOperator "\s[&^|~]=\s"
syn match customAssignOperator "\s[-+*/%]=\s"

syn match customMinusOperator "-[0-9]\+"
syn match customMinusOperator "-("me=e-1

syn match customAddOperator "&[^&= ]"me=s+1
syn match customAddOperator "*[^= ]"me=s+1
syn match customAddOperator "->"
syn match customAddOperator "\.[a-zA-Z_]"me=s+1
...
hi customAnsiFunction ctermbg=none cterm=bold,italic ctermfg=Yellow
hi cAnsiName ctermbg=none cterm=bold,italic ctermfg=Green
hi Function cterm=bold ctermfg=Yellow
hi customArithOperator cterm=bold ctermfg=LightRed
hi customBitOperator cterm=bold ctermfg=LightRed
hi customComOperator cterm=bold ctermfg=Red
hi reverseOperator cterm=bold ctermfg=Red
hi customAssignOperator  cterm=bold ctermfg=LightRed
hi customMinusOperator cterm=bold ctermfg=Magenta
hi customAddOperator cterm=bold ctermfg=Cyan
hi Delimiter cterm=bold ctermfg=Gray
